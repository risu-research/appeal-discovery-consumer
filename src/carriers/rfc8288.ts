...
function isRelationType(value: string): boolean {
  if (REGISTERED_RELATION.test(value)) return true;
  try {
    new URL(value);
    return true;
  } catch {
...
function splitDelimited(
  ...
  if (quoted || escaped || inAngle) return undefined;
  parts.push(input.slice(start));
  return parts;
}

function decodeParameterValue(raw: string): string | undefined {
...
function parseLinkValue(value: string): ParsedLink | undefined {
...
function extensionRelationEquals(left: string, right: string): boolean {
...
export function discoverRfc8288(
  ...
  for (const header of input.linkHeaders) {
    ...
    for (const value of values) {
      ...
      if (
        link === undefined ||
        !link.relations.some((relation) =>
          extensionRelationEquals(relation, semantic),
        )
      ) {
        continue;
      }
      ...
      if (target === undefined || context === undefined) continue;

      discovered.push(
        result(
          semantic,
          "rfc8288",
          { kind: "uri", value: context },
          { kind: "link", target },
          input.source,
        ),
      );
