...
ACTION_KEYS = {
    "schema", "capture_point", "clock_domain", "publish_mono_ns",
    "topic", "payload_utf8", "qos", "retain", "properties",
}
ALLOWED_IMPORT_ROOTS = {
    "argparse", "ast", "collections", "hashlib", "json", "pathlib", "subprocess",
}
...
def identity(device, role):
    if (
        ...
        or any(ch in device for ch in ("+", "#", "\x00"))
    ):
...
def literal_safe(obs, action):
    if (
        ...
        or set(obs) != OBS_KEYS
        or obs["schema"] != "replaymark.runtime.e3b-observation-record.v1"
    ):
    ...
    command, deadline, closed = map(
        ns,
        (
            obs["command_publish_mono_ns"],
            obs["verify_deadline_mono_ns"],
            obs["closed_at_mono_ns"],
        ),
    )
    ...
    for event in events:
        ...
        if (
            event["clock_domain"] != clock
            or recv > closed
            or not isinstance(event["on"], bool)
        ):
            raise ValueError("event timing/type drift")
        if (
            event["device"] == obs["expected_device"]
    ...
    if (
        ...
        or action["schema"] != "replaymark.runtime.e3b-historical-action-record.v1"
    ):
        raise ValueError("action schema drift")
    if (
        action["capture_point"] != "paho.mqtt.Client.publish.pre-send"
        or action["clock_domain"] != clock
    ):
    ...
    if (
        action["payload_utf8"] != '{"on": true}'
        or action["qos"] != 1
        or isinstance(action["qos"], bool)
        or action["retain"] is not False
        or action["properties"] is not None
    ):
...
def policy_for(trial, task):
    ...
    digest = hashlib.sha256(
        f"{POLICY_SEED}|trial={trial}|block={block}".encode()
    ).digest()
...
def reference_blocks(trial):
    ranked = sorted(
        (
            hashlib.sha256(
                f"{CLASS_SEED}|trial={trial}|block={block}".encode()
            ).hexdigest(),
            block,
        )
...
def effect_count(row, prefix, task):
    ...
    after = ns(action["publish_mono_ns"])
    deadline = ns(row["b_effect_deadline_mono_ns"])
    ...
    for event in effects:
        if not isinstance(event, dict):
            raise ValueError("malformed B effect")
        recv = event.get("recv_mono_ns")
        if (
            event.get("device") == expected
            and event.get("on") is True
            and event.get("cause") == "command"
            and isinstance(recv, int)
            and after <= recv <= deadline
        ):
            count += 1
    return count
...
def preflight():
    ...
    changed = set(
        git("diff", "--name-only", RESULT_SEAL_HEAD, "HEAD").splitlines()
    )
    if changed != EXPECTED_INCREMENT:
        raise AssertionError(
            f"undeclared S2-F increment: missing={sorted(EXPECTED_INCREMENT-changed)} "
    ...
    seal = json.loads(
        (
            ROOT
            / "replaymark"
            / "S2P6_HETEROGENEOUS_TARGET_CAPSTONE_RESULT_SEAL.json"
        ).read_text()
    )
    if (
        seal.get("status") != "FROZEN_FIRST_COMPLETE_PASS"
        or seal["authority"]["execution_head"] != EXECUTION_HEAD
    ):
        raise AssertionError("P6 result authority drift")
    if (
        seal["artifact"]["id"] != ARTIFACT_ID
        or seal["artifact"]["sha256"] != ARTIFACT_SHA256
    ):
    ...
    if not roots <= ALLOWED_IMPORT_ROOTS:
        raise AssertionError(
            f"second literal auditor imports non-stdlib roots: "
    ...
    workflow = (
        ...
        / ".github"
        / "workflows"
        / "replaymark-runtime-gate-s2f-heterogeneous-target-audit.yml"
    ).read_text()
    for token in (
        "docker compose",
        "s2p6_heterogeneous_device.py --broker",
        "replaymark_s2p6_heterogeneous_capstone.py --broker",
    ):
    ...
    for required in (str(ARTIFACT_ID), str(ARTIFACT_RUN), ARTIFACT_SHA256):
        if required not in workflow:
            raise AssertionError(
                f"workflow not pinned to source authority: {required}"
            )
    return {
        "exact_result_seal_parent": True,
        "declared_files_only": True,
        "stdlib_only_literal_auditor": True,
        "new_live_execution": False,
        "source_artifact_pinned": True,
    }


def audit(source_dir):
    raw_path = source_dir / "s2p6_live_raw.json"
    original_verify_path = source_dir / "p6_final_verification.json"
    original_preflight_path = source_dir / "p6_preflight.json"
    seal_commit_path = source_dir / "SEAL_COMMIT.txt"
    sums_path = source_dir / "SHA256SUMS"
    for path in (
        raw_path,
        original_verify_path,
    ...
    if EXECUTION_HEAD not in seal_commit_path.read_text():
        raise AssertionError("artifact commit binding drift")
    if json.loads(original_verify_path.read_text()).get("verdict") != "PASS":
        raise AssertionError("source artifact original verification is not PASS")
    if json.loads(original_preflight_path.read_text()).get("verdict") != "PASS":
        raise AssertionError("source artifact original preflight is not PASS")
    ...
    if (
        report.get("schema")
        != "replaymark.s2p6-live-heterogeneous-target-capstone.v1"
        or report.get("broker") != "mosquitto version 2.1.2"
    ):
        raise AssertionError("live report identity drift")
    if (
        report.get("fallback"),
        report.get("regeneration"),
        report.get("automatic_retry"),
    ...
    if report.get("harness_closed_before_report") is not True:
        raise AssertionError("live harness was not closed before report")
    ...
    if report.get("historical_template_sha256") != seal.get(
        "historical_template_sha256"
    ):
        raise AssertionError("historical template digest drift")

    calls = report.get("recording_client_calls")
    if not isinstance(calls, list):
        raise AssertionError("recording calls missing")
    calls_by_action = Counter(
        (
            call.get("topic"),
            call.get("payload"),
            call.get("qos"),
    ...
    if any(count != 1 for count in calls_by_action.values()):
        raise AssertionError("duplicate exact application action")

    safe_count = Counter()
    unsafe_count = Counter()
    executed_count = Counter()
    class_truth = Counter()
    policy_class = defaultdict(Counter)
    assigned = defaultdict(set)
    safe_sets = defaultdict(set)
    executed_sets = defaultdict(set)
    trial_truth = defaultdict(Counter)
    reference_unsafe_replaymark = []
    semantic_mismatch = []
    execution_mismatch = []
    phase_failure = []
    effect_failure = []
    call_failure = []
    candidate_action_keys = set()
    prefixes = set()
    rows_seen = 0
    ...
    for trial_index, trial in enumerate(trials):
        if (
            ...
            or not isinstance(trial.get("rows"), list)
            or len(trial["rows"]) != 192
        ):
        ...
        if not isinstance(prefix, str) or not prefix or prefix in prefixes:
            raise AssertionError("trial prefix is not fresh")
        prefixes.add(prefix)

        freeze, quiescence, dispatch_done, effects_done = map(
            ns,
            (
                trial["observation_freeze_complete_mono_ns"],
                trial["a_quiescence_barrier_mono_ns"],
                trial["dispatch_complete_mono_ns"],
                trial["b_effect_barrier_mono_ns"],
            ),
        )
        ...
        for task, row in enumerate(trial["rows"]):
            ...
            safe, parsed_prefix, parsed_task = literal_safe(
                row["raw_observation"], row["raw_action"]
            )
            if parsed_task != task or parsed_prefix != prefix:
                raise AssertionError("row/candidate identity drift")
            key = (trial_index, task)
            assigned[policy].add(key)
            if safe:
                safe_count[policy] += 1
                safe_sets[policy].add(key)
                class_truth[f"{cls}_safe"] += 1
                trial_truth[trial_index]["safe"] += 1
            else:
                unsafe_count[policy] += 1
                class_truth[f"{cls}_unsafe"] += 1
                trial_truth[trial_index]["unsafe"] += 1

            expected_verdict = "VALID" if safe else "INVALID"
            expected_reuse = "REUSE" if safe else "DO_NOT_REUSE"
            if (
                row.get("certificate_verdict") != expected_verdict
                or row.get("certificate_reuse_disposition") != expected_reuse
            ):
                semantic_mismatch.append(key)

            exact = action_key(row["raw_action"])
            candidate_action_keys.add(exact)
            ...
            if actual_exec:
                executed_count[policy] += 1
                executed_sets[policy].add(key)
            if row.get("executed") is not actual_exec:
                execution_mismatch.append(key)

            obs_closed = ns(row["raw_observation"]["closed_at_mono_ns"])
            action_stamp = ns(row["raw_action"]["publish_mono_ns"])
            if obs_closed > freeze:
                phase_failure.append((trial_index, task, "snapshot_after_freeze"))
            if not quiescence <= action_stamp <= dispatch_done:
                phase_failure.append((trial_index, task, "candidate_phase_order"))
            ...
            if actual_exec:
                ...
                if len(matching) != 1:
                    call_failure.append((trial_index, task, "sink_cardinality"))
                else:
                    sink_stamp = ns(matching[0]["stamp_mono_ns"])
                    if not quiescence <= sink_stamp <= dispatch_done:
                        phase_failure.append(
                            (trial_index, task, "sink_phase_order")
                        )

            effects = effect_count(row, parsed_prefix, task)
            if actual_exec and effects < 1:
                effect_failure.append((trial_index, task, "executed_no_effect"))
            if not actual_exec and effects != 0:
                effect_failure.append((trial_index, task, "blocked_has_effect"))

            if policy == "REPLAYMARK" and cls == "REFERENCE" and not safe:
                reference_unsafe_replaymark.append(key)

    for trial in range(6):
        if trial_truth[trial]["safe"] == 0 or trial_truth[trial]["unsafe"] == 0:
            raise AssertionError(f"trial {trial} is not semantically mixed")
        for policy in POLICIES:
            if policy_class[(trial, policy)] != Counter(
                {"REFERENCE": 32, "SHIFTED": 32}
            ):
                raise AssertionError(
                    f"policy/class orthogonality drift trial={trial} policy={policy}"
                )

    foreign_calls = set(calls_by_action) - candidate_action_keys
    expected = seal["first_complete_result"]
    safe = {policy: safe_count[policy] for policy in POLICIES}
    unsafe = {policy: unsafe_count[policy] for policy in POLICIES}
    executed = {policy: executed_count[policy] for policy in POLICIES}
    by_class = {
    ...
    checks = {
        ...
        "fresh_trial_prefixes": len(prefixes) == 6,
        "recording_calls_match_report_count":
            len(calls) == report.get("recording_client_call_count"),
        "recording_sink_has_no_foreign_actions": not foreign_calls,
        "production_semantics_match_second_literal_auditor":
            not semantic_mismatch,
        "row_execution_matches_actual_sink": not execution_mismatch,
        "phase_order_exact": not phase_failure,
        "B_effects_consistent": not effect_failure,
        "action_call_mismatches_zero": not call_failure,
        "always_admit_all":
            executed_sets["ALWAYS_ADMIT"] == assigned["ALWAYS_ADMIT"],
        "always_admit_unsafe_positive":
            bool(executed_sets["ALWAYS_ADMIT"] - safe_sets["ALWAYS_ADMIT"]),
        "always_block_empty": not executed_sets["ALWAYS_BLOCK"],
        "always_block_discards_safe_positive": bool(safe_sets["ALWAYS_BLOCK"]),
        "replaymark_exact_literal_safe_set":
            executed_sets["REPLAYMARK"] == safe_sets["REPLAYMARK"],
        "class_only_reference_rule_falsified_on_replaymark_arm":
            len(reference_unsafe_replaymark) > 0,
        "replaymark_blocks_all_reference_but_unsafe":
            all(
                key not in executed_sets["REPLAYMARK"]
                for key in reference_unsafe_replaymark
            ),
        "gate_consumed_384":
            report.get("replaymark_gate_consumed_count") == 384,
        "sealed_safe_counts_reproduced":
            safe == expected["safe_by_policy"],
        "sealed_unsafe_counts_reproduced":
            unsafe == expected["unsafe_by_policy"],
        "sealed_executed_counts_reproduced":
            executed == expected["executed_by_policy"],
        "sealed_class_counts_reproduced":
            by_class == expected["oracle_by_hidden_target_class"],
    }
    ...
    return {
        ...
        "recomputed": {
            ...
            "replaymark_reference_but_unsafe_tasks": [
                list(key) for key in reference_unsafe_replaymark
            ],
            "class_only_reference_rule_unsafe_executions_on_replaymark_arm":
                len(reference_unsafe_replaymark),
        },
        "failures": {
            "semantic": semantic_mismatch,
            "execution": execution_mismatch,
            "phase": phase_failure,
            "effects": effect_failure,
            "calls": call_failure,
            "foreign_sink_calls": [list(item) for item in foreign_calls],
        },
        "source": {
            "artifact_id": ARTIFACT_ID,
            "workflow_run": ARTIFACT_RUN,
            "artifact_zip_sha256": ARTIFACT_SHA256,
            "live_raw_sha256": sha256_file(raw_path),
        },
    }
...
def main():
    ...
    if args.preflight_only:
        report = {
            "schema": "replaymark.s2f-audit-preflight.v1",
            "verdict": "PASS",
            "preflight": pre,
        }
    else:
