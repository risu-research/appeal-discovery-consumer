...
def main() -> None:
    ...
    # Detached but byte-identical values must take the original content-addressed path.
    detached_observation = replace(pair.observation)
    detached_action = replace(pair.historical_action)
    detached_pair = replace(
        pair,
        observation=detached_observation,
        historical_action=detached_action,
    )
    ...
    assert detached_semantic.observation is not pair.observation
    assert detached_semantic.historical_action is not pair.historical_action
    detached, detached_counts = _count_wrapper_fingerprints(
        detached_pair,
        detached_semantic,
    )
    ...
    # All three semantic outcomes must preserve certificate bytes and the fast path.
    outcome_rows = {}
