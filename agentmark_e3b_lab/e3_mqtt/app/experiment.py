...
class Harness:
    def __init__(self,broker,port):
        ...
        self.client=mqtt.Client(mqtt.CallbackAPIVersion.VERSION2,client_id=f'agentmark-{uuid.uuid4().hex[:8]}'); self.client.on_connect=self._on_connect; self.client.on_message=self._on_message; self.client.connect(broker,port,keepalive=30); self.client.loop_start()
        d=time.monotonic()+10
        while not self.client.is_connected():
            if time.monotonic()>d:raise TimeoutError('runner connect')
            time.sleep(.01)
    def close(self): self.client.disconnect(); self.client.loop_stop()
    def _on_connect(self,c,u,f,r,p): c.subscribe('agentmark/+/state',qos=1); c.subscribe('agentmark/control/ack',qos=1); c.subscribe('$SYS/broker/#',qos=0)
    ...
    def wait_state_on_until(self,device,deadline_ns,*,after_ns):
        with self.cv:
            while True:
                ...
                if r<=0:return None
                self.cv.wait(r/1e9)
    def wait_sys_after(self,after_ns,timeout_s=2.5):
        d=time.monotonic()+timeout_s
        with self.cv:
            while True:
                x=self.sys_values.get('$SYS/broker/messages/received')
                if x is not None and x[1]>after_ns:return
                r=d-time.monotonic()
                if r<=0:return
                self.cv.wait(r)
    def fresh_sys_snapshot(self):
        s=time.monotonic_ns(); self.wait_sys_after(s); return self.sys_counter_snapshot()
    def sys_counter_snapshot(self):
        wanted=('$SYS/broker/messages/received','$SYS/broker/messages/sent','$SYS/broker/publish/messages/received','$SYS/broker/publish/messages/sent','$SYS/broker/publish/bytes/received','$SYS/broker/publish/bytes/sent','$SYS/broker/bytes/received','$SYS/broker/bytes/sent'); out={}
        with self.cv:
            for k in wanted:
                if k in self.sys_values:
